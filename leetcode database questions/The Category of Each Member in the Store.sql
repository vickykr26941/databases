...
select a.member_id,a.name,(
    case
     when (100 * count(c.visit_id)) / count(b.member_id) >= 80 then "Diamond"
     when (100 * count(c.visit_id)) / count(b.member_id) >= 50 then "Gold"
     when (100 * count(c.visit_id)) / count(b.member_id) < 50 then "Silver"
     when (100 * count(c.visit_id)) / count(b.member_id) is null then "Bronze" 
    end
) as category
from members a left join visits b using(member_id) left join purchases c using(visit_id)
