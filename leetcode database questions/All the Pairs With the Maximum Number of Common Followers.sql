...
select first_val user1_id,second_val user2_id from (
    select a.user_id first_val,b.user_id second_val,count(a.follower_id) cnt from relations a,relations b
...
-- using with 
with tmp as (
    select a.user_id x,b.user_id y,count(*) cnt from relations a, relations b
    where a.follower_id = b.follower_id and a.user_id < b.user_id
    group by a.user_id,b.user_id
)
select x user1_id,y user2_id from tmp
where cnt = (select max(cnt) cnt from tmp);
